...
export async function generateMetadata() {
  ...
  const title = seo.title || tenant?.name || "Tourism Agency";
  const description = seo.description || "Discover destinations and book your next experience.";
...
export default async function RootLayout({ children }) {
  ...
  const tenant = data?.tenant;
  const tenantName = tenant?.name || "Vanguard Tourism";
